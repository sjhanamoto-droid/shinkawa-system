...
import { audienceRoleCounts, claimPropertyOptions } from "@/features/claims/queries";
...
export default async function NewClaimPage({ searchParams }: { searchParams: Promise<{ propertyId?: string }> }) {
  ...
  const [properties, roleCounts] = await Promise.all([claimPropertyOptions(), audienceRoleCounts(me.id)]);
  return (
    <div>
      <PageHeader title="クレームを登録・共有" backHref="/claims" />
      <PageContainer size="narrow">
        <ClaimForm
          initial={{ title: "", propertyId: properties.some((p) => p.value === propertyId) ? propertyId! : "", occurredOn: "", content: "", cause: "", prevention: "" }}
