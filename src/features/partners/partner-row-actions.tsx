...
export function PartnerRowActions({ id, active, canDelete }: { id: string; active: boolean; canDelete: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <button
        type="button"
        disabled={pending}
        onClick={() => start(async () => { const r = await togglePartnerActive(id); if (r.error) toast(r.error, { type: "error" }); })}
      ...
      {canDelete && (
        <button
          ...
          onClick={() => {
            ...
            start(async () => { const r = await deletePartner(id); if (r && "error" in r && r.error) toast(r.error, { type: "error" }); });
          }}
          aria-label="削除"
